<script lang="ts">
  ...
  const onRangeChange = async (value: string) => {
    ...
    await goto(`/dashboard/songs?${params.toString()}`, {
      keepFocus: true,
      noScroll: true,
    });
  };
...
<div class="space-y-4">
  <div class="flex items-center justify-between">
    <div>
      ...
      <p class="text-sm text-muted-foreground">
        {songs.length} songs in {currentLabel.toLowerCase()}
      </p>
    </div>
  ...
  <Card.Root>
    <Card.Header>
      <Card.Title>Song Rankings</Card.Title>
      <Card.Description>Ranked by total plays</Card.Description>
    </Card.Header>
    <Card.Content>
      {#if songs.length === 0}
        <p class="text-sm text-muted-foreground">
          No song data found for this range.
        </p>
      {:else}
        <div class="space-y-2">
          {#each songs as song, index}
            ...
            >
              <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                  <div class="truncate font-medium">{song.song}</div>
